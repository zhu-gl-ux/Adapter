...
class PolyLR(_LRScheduler):
    ...
    def get_lr(self):
        # lr = [base_lr * (1 - self.last_epoch / self.max_iters)**self.power
        #         for base_lr in self.base_lrs]
        # print(lr)
        # lr[1:-1] = lr[1:-1].clamp_min(torch.tensor(self.base_lrs[0]))
        # print(lr)
        return [base_lr * (1 - self.last_epoch / self.max_iters)**self.power
                for base_lr in self.base_lrs]
        # return [base_lr*torch.cos(torch.tensor(math.pi/2*self.last_epoch / self.max_iters)) for base_lr in self.base_lrs]
